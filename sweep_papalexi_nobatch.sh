#!/usr/bin/env bash
# Parameter sweep for Papalexi dataset CVAE — bio_rep and lane excluded from covariates.
# z is free to capture batch variation; phase is still conditioned out.
# All 20 result subdirectories are placed under results_papalexi_nobatch/.
# Usage: bash sweep_papalexi_nobatch.sh

CONDA_PREFIX="/project/xuanyao/jiaming/miniconda3/envs/cvae_perturb"
PYTHON="${CONDA_PREFIX}/bin/python"
BASE_DIR="/project/xuanyao/jiaming/deeplearning"
SWEEP_DIR="${BASE_DIR}/results_papalexi_nobatch"
...
for BETA in "${BETAS[@]}"; do
  for DIM in "${DIMS[@]}"; do
    ...
    sbatch --job-name="cvae_papalexi_nobatch_${TAG}" \
      --account=pi-xuanyao \
...
echo '=== Train: beta=${BETA} latent_dim=${DIM} (no bio_rep, no lane) ==='
