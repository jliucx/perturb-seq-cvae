#!/usr/bin/env bash
# Parameter sweep for Papalexi dataset CVAE.
# Sweeps beta (KL weight) and latent_dim.
# All 20 result subdirectories are placed under results_papalexi/.
# Usage: bash sweep_papalexi.sh

CONDA_PREFIX="/project/xuanyao/jiaming/miniconda3/envs/cvae_perturb"
PYTHON="${CONDA_PREFIX}/bin/python"
BASE_DIR="/project/xuanyao/jiaming/deeplearning"
SWEEP_DIR="${BASE_DIR}/results_papalexi"
...
for BETA in "${BETAS[@]}"; do
  for DIM in "${DIMS[@]}"; do
    ...
    sbatch --job-name="cvae_papalexi_${TAG}" \
      --account=pi-xuanyao \
...
echo '=== Train: beta=${BETA} latent_dim=${DIM} ==='
