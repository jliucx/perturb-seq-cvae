#!/usr/bin/env bash
# Parameter sweep for Papalexi dataset CVAE — perturbation one-hot zeroed out (ablation).
# Model sees only expression + covariates; tests if latent space still separates perturbations.
# All 20 result subdirectories are placed under results_papalexi_no_pert/.
# Usage: bash sweep_papalexi_no_perturbation.sh

CONDA_PREFIX="/project/xuanyao/jiaming/miniconda3/envs/cvae_perturb"
PYTHON="${CONDA_PREFIX}/bin/python"
BASE_DIR="/project/xuanyao/jiaming/deeplearning"
SWEEP_DIR="${BASE_DIR}/results_papalexi_no_pert"
...
for BETA in "${BETAS[@]}"; do
  for DIM in "${DIMS[@]}"; do
    ...
    sbatch --job-name="cvae_papalexi_nopt_${TAG}" \
      --account=pi-xuanyao \
...
echo '=== Train: beta=${BETA} latent_dim=${DIM} (no perturbation) ==='
